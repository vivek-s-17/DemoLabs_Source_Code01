 ...
 *  (a) Identity Column
 *  (b) Constraint - Primary Key (Clustered Index)
 *  (c) Constraint - Check Constraint
 *  (d) Constraint - Default Constraint
 *  (e) Constraint - Unique Constraint (Non-Clustered Index)
 ******************************************************/
 ...
 CREATE TABLE [Persons] (
    [ID] smallint NOT NULL IDENTITY(1,1)							-- autogenerated value on new row
    , [Name] nvarchar(50) NOT NULL
    , [Age] int NULL
	, [Country] varchar(50) NOT NULL
	, [City] varchar(50) NOT NULL
	, [TaxID] varchar(20) NOT NULL
		CONSTRAINT [UQ_Email] UNIQUE								-- alternate key (automatic non-clustered index)

	, CONSTRAINT [PK_Persons] 			PRIMARY KEY ( [ID] ASC )	-- primary key (automatic clustered index)
	, CONSTRAINT [CHK_Persons_Country] 	CHECK ( [Country] IN ('India', 'USA', 'UK') )
	, CONSTRAINT [DF_Persons_Country]	DEFAULT 'India' FOR [Country]
	, CONSTRAINT [DF_Persons_City] 		DEFAULT 'Mumbai' FOR [City]
	, CONSTRAINT [CHK_Persons_Age] 		CHECK ( [Age] >= 18 )
	, CONSTRAINT [UQ_Persons_Name]		UNIQUE ( [Name] )			-- alternate key (automatic non-clustered index)
	...
	*/
	...
	, INDEX [IX_Persons_City]			NONCLUSTERED ( [City] )		-- non-unique non-clustered index
 )
 GO

 INSERT INTO [Persons]
	( [Name], [Age], [Country], [TaxID] )
 VALUES
	( N'First Person', 28, 'India', 'Tax01' )
	, ( N'Second Person', 35, 'UK', 'Tax02' )
 GO
